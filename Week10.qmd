...
## Ensemble Trees: Bagging and Random Forests

- Numeric Response: MLR, Penalized MLR (LASSO), Regression Trees

- Categorical Response: Logistic Regression (Log-Loss) and Classification Trees

- Ensemble tree methods:

  - Bagging (bootstrap aggregation)

  - Random Forests (extension of bagging)

  - Boosting (slow training of trees)

- Parametric: data comes from model distribution (i.e. with parameters like mean, stdev, etc.)

- Non-Parametric: treats the sample as the population, and can resample with replacement
...
# Note:  Response variable is HeartDisease, which is binary
```

```{r}
# We can use the same recipe from our linear regression
...
# Note:  cross validation is not really needed because of how bagging works
bag_fit <- bag_wkf |>
...
```{r}
# check our metrics across the tuned parameter cost_complexity
...
```{r}
# get the best tuning parameter (cost_complexity )
...
- For comparison, consider the Log Regresion model from before
...
- Now Random Forests
  ...
  - Similar to bagging, except the splits of the trees are considered using a random subset of the predictors each time
  
  - Allows for consideration of more predictors and better predictions overall

```{r}
# set up random forest model
library(ranger)
rf_spec <- rand_forest(mtry = tune()) |> # mtry: number of predictors is the tuning parameter
  set_engine("ranger") |>
  set_mode("classification")
```

```{r}
# use same workflow as before
rf_wkf <- workflow() |>
  add_recipe(LR3_rec) |>
  add_model(rf_spec)
```
...
```{r}
# again fitting with CV, although CV is not needed for bagging and forests
rf_fit <- rf_wkf |>
  tune_grid(resamples = heart_CV_folds, 
            grid = tibble(mtry = 1:7), # trying values 1 to 7 of mtry
            metrics = metric_set(accuracy, mn_log_loss))
```
